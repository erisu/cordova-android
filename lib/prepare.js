...
module.exports.prepare = function (cordovaProject, options) {
    ...
    return Promise.resolve(updateWww(cordovaProject, this.locations))
        .then(() => updateSplashScreenBySplashTag(this.locations, cordovaProject))
...
function updateSplashScreenBySplashTag (locations, cordovaProject) {
    const splash = cordovaProject.projectConfig.doc.findall('./platform[@name="android"]/splash') || [];

    // Skip if no <splash> tags exists. Default will be handled later.
    if (splash.length < 1) {
        return;
    }

    // No longer support of multiple splash screen.
    if (splash.length > 1) {
        events.emit('warn', 'More then one "<splash>" tag was detected. Cordova no longer support defining of multiple SplashScreens. Please update to a singular SplashScreen design.');
    }

    // Update windowSplashScreenAnimatedIcon based on <splash> value or fall back to default.
    const themeKey = 'windowSplashScreenAnimatedIcon';
    const splashSrc = splash[0].attrib.src;
    updateProjectSplashScreenImage(locations, themeKey, '<splash>', splashSrc);

    // Update global flag that there was a splash tag.
    hasSplashTag = true;
}
...
function updateProjectSplashScreenImage (locations, themeKey, cdvConfigPrefKey, cdvConfigPrefValue = '') {
    ...
    if (
        hasSplashTag && themeKey === 'windowSplashScreenAnimatedIcon' &&
        !(cdvConfigPrefValue && fs.existsSync(cdvConfigPrefValue))
    ) {
        return;
    }
